...
float traceRayToSphere(float3 rayOrigin, float3 rayDirection, float4 sphere) {
    // Attempt to intersect the ray with the sphere defined by returnSPH()
    return sphereIntersect(rayOrigin, rayDirection, sphere);
}
...
float adjustVerticalPositionEarth(float3 position, float4 sphere) {
    float2 positionOffset = position.xz - sphere.xz; // Offset in the horizontal plane
...
float adjustVerticalPosition(float3 position) {
    ...
    for (unsigned int i = 0; i < 3; ++i) { // Iterate over planets
